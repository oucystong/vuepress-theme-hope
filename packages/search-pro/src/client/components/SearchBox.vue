...
export default defineComponent({
  ...
  setup() {
    ...
    const options = __SEARCH_PRO_OPTIONS__;
...
<style lang="scss">
$next-search-input-accent-border-color: var(
  --next-search-input-accent-border-color
);
...
.search-box {
  ...
  input {
    ...
    display: inline-block;
    //border: 1px solid var(--next-search-input-border-color);
    border: 1px solid var(--next-search-input-border-color);
    border-radius: 2rem;
    font-size: 0.9rem;
    line-height: 2rem;
    ...
    background: $next-search-bg-color url("../assets/search.svg") 0.6rem 0.5rem
      no-repeat;
  ...
  .suggestions {
    ...
    position: absolute;
    top: 1.5rem;
    border: 1px solid var(--next-search-border-color);
    padding: 0.4rem;
